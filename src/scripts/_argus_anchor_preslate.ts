/**
 * Argus-Anchor pre-slate: Atlas + anchor-concentration architecture.
 *
 * Same architecture as Atlas (t-copula, EV-vs-uniform-field, GBM v3 combo prior)
 * EXCEPT the flat exposure caps are replaced with anchor-aware caps:
 *
 *   - Top-N_HIT hitters by projection: cap raised from default 0.25 to ANCHOR_HIT_CAP (0.55)
 *   - Top-N_PIT pitchers by projection: cap raised from default 0.45 to ANCHOR_PIT_CAP (0.70)
 *   - All other players: existing flat caps preserved
 *
 * Motivation: live-audit (live_audit/FINDINGS.md, 2026-05-16) showed pros routinely
 * concentrate beyond Atlas's caps (78% of contests had pro max-hitter > 25%, median
 * pro max-hitter = 32%, p75 = 46%, p90 = 58%). Atlas's flat caps structurally
 * prevent winning concentration on slate-anchor chalk plays.
 *
 * Env vars:
 ...
 *   ARGUS_OUTPUT_TAG    default 'anchor'
...
const TAG = process.env.ARGUS_OUTPUT_TAG || 'anchor';
...
const ANCHOR_BOOST = process.env.ARGUS_ANCHOR_BOOST ? parseFloat(process.env.ARGUS_ANCHOR_BOOST) : 5.0; // $ added per anchor in lineup
...
async function main() {
  console.log('================================================================');
  console.log('ARGUS-ANCHOR pre-slate (Atlas + anchor-concentration caps)');
  console.log('================================================================');
  ...
  for (let step = 0; step < N; step++) {
    ...
    for (let gi = 0; gi < greedyPool.length; gi++) {
      ...
      if (!stackOk) continue;
      // Count anchors in this lineup for objective boost.
      let nAnchors = 0;
  ...
  console.log('\n================================================================');
  console.log('PORTFOLIO STATS — Argus-Anchor');
  console.log('================================================================');
  ...
  const outDk = path.join(DIR, `theory_dfs_argus_anchor_preslate_${selected.length}${tagSuffix}.csv`);
  const outDetail = path.join(DIR, `theory_dfs_argus_anchor_preslate_${selected.length}${tagSuffix}_detailed.csv`);
  exportForDraftKings(selLineups, config, outDk);
  exportDetailedLineups(selLineups, config, outDetail);
  console.log(`\n================================================================`);
  console.log(`DONE — Argus-Anchor preslate`);
  console.log(`================================================================`);
