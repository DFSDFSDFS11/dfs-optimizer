/**
 * Atlas vs Stacks-BB vs Stacks-V3 (proj_min + n_lev filters) — 29-slate LOO backtest.
 *
 ...
 *
 * Stacks-V3 = Stacks-BB + proj_min hard floor (4.0 pts) + n_lev_5less cap (max 2 punts).
 *
...
const OUT_JSON = path.join(DIR, 'atlas_vs_stacks_v3_results.json');
...
// V3 params (WFA-validated)
const V3_PROJ_MIN_FLOOR = parseFloat(process.env.V3_PROJ_MIN_FLOOR || '4.0');  // reject lineups with any player below this
const V3_MAX_LEV_5 = parseInt(process.env.V3_MAX_LEV_5 || '2');  // max # of players < 5% owned
...
// ===== Stacks-V3 selector (Atlas + bring-back floor + IC composite score) =====
...
function selectStacksICPortfolio(input: AtlasInput): Lineup[] {
  ...
  for (let step = 0; step < N; step++) {
    ...
    for (let gi = 0; gi < greedyPool.length; gi++) {
      ...
      // V3: hard filters on proj_min and n_lev_5
      if (candProjMin[c] < V3_PROJ_MIN_FLOOR) continue;
      if (candNLev5[c] > V3_MAX_LEV_5) continue;
      const lu = candidates[c];
      let okExp = true;
      for (const p of lu.players) {
        const isAnchor = anchorIds.has(p.id);
...
async function main() {
  console.log('================================================================');
  console.log('Atlas vs Stacks-BB vs Stacks-V3 — 29-slate LOO three-way backtest');
  console.log('================================================================\n');
  ...
  console.log('Metric                       Atlas         Stacks-BB         Stacks-V3');
  console.log('---------------------------------------------------------------------------');
  ...
  console.log('Metric                       pro tgt    Atlas              Stacks-BB         Stacks-V3');
  for (const k of UNIVERSAL_METRICS) {
  ...
  console.log('================================================================\n');
  for (const [label, a, l] of [['Atlas', aAgg, aLoo] as const, ['Stacks-BB', vAgg, vLoo] as const, ['Stacks-V3', iAgg, iLoo] as const]) {
