...
interface SharedInput {
  ...
  // Per-candidate additional features
  candTopGameStack: Int8Array;  // # of hitters from top-saber-game-total game
  candDevSumUnder: Float32Array;  // sum of negative (Adj Own - implied own)
...
interface VariantConfig {
  ...
  devSumUnderBias: number;  // $ boost per unit of negative deviation (per Theory Ch.5 RV)
}
...
function selectVariant(shared: SharedInput, config: VariantConfig): Lineup[] {
  ...
  for (let step = 0; step < N; step++) {
    ...
    for (let gi = 0; gi < greedyPool.length; gi++) {
      ...
      const score = candEV[c] - ATLAS_W_MULTI_BLEND * chalkRank[c] + STACKS_ANCHOR_BOOST * nAnchors + devBonus;
...
async function main() {
  ...
  const VARIANTS: VariantConfig[] = [
    { name: 'Atlas',     bb1Target: 0,    bb2Target: 0,    bb3Target: 0,    gameStackFloor: 0,    gameStackMinHitters: 4, devSumUnderBias: 0 },
    { name: 'BB17',      bb1Target: 0.17, bb2Target: 0.08, bb3Target: 0,    gameStackFloor: 0,    gameStackMinHitters: 4, devSumUnderBias: 0 },
    { name: 'BB25',      bb1Target: 0.25, bb2Target: 0.12, bb3Target: 0.05, gameStackFloor: 0,    gameStackMinHitters: 4, devSumUnderBias: 0 },
    { name: 'BBM3',      bb1Target: 0.17, bb2Target: 0.08, bb3Target: 0.04, gameStackFloor: 0,    gameStackMinHitters: 4, devSumUnderBias: 0 },
    { name: 'GAME',      bb1Target: 0.17, bb2Target: 0.08, bb3Target: 0,    gameStackFloor: 0.25, gameStackMinHitters: 4, devSumUnderBias: 0 },
    { name: 'DEV',       bb1Target: 0.17, bb2Target: 0.08, bb3Target: 0,    gameStackFloor: 0,    gameStackMinHitters: 4, devSumUnderBias: 0.3 },
    { name: 'MAX',       bb1Target: 0.17, bb2Target: 0.08, bb3Target: 0.04, gameStackFloor: 0.20, gameStackMinHitters: 4, devSumUnderBias: 0.2 },
  ];
