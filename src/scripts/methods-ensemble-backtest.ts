/**
 * Atlas vs Stacks-BB vs Stacks-V4 (proj_min + n_lev filters) — 29-slate LOO backtest.
 *
 * WFA-validated IC findings (2026-05-16 wfa_ic_pipeline run):
 *   - proj_min: DEV IC +0.112, WFA +0.047, 82% sign-stable (MOST STABLE)
 *   - h_proj_min: DEV IC +0.111, WFA +0.056, 82% sign-stable
 *   - n_lev_5less: DEV IC -0.077, WFA -0.092 (negative — too many leverage plays hurt)
 *   - n_lev_10less: DEV IC -0.072, WFA -0.104 (growing more negative)
 *   - dev_sum_under: DEV IC +0.107, WFA +0.044 (under-owned-vs-implied is positive signal)
 *
 * Stacks-V4 = Stacks-BB + proj_min hard floor (4.0 pts) + n_lev_5less cap (max 2 punts).
 *
 * Per Method 4: this is the IS run on full 29 slates for comparison.
 *
...
const OUT_JSON = path.join(DIR, 'atlas_vs_stacks_v4_results.json');
...
// V4 params: SOFT penalty version (less rigid than V3 hard filter)
const V4_PROJ_MIN_PENALTY = parseFloat(process.env.V4_PROJ_MIN_PENALTY || '5.0');  // $ subtracted per point below 4.0
const V4_NLEV_PENALTY = parseFloat(process.env.V4_NLEV_PENALTY || '8.0');  // $ subtracted per <5% own player above max=2
const V3_PROJ_MIN_FLOOR = parseFloat(process.env.V3_PROJ_MIN_FLOOR || '0.0');  // disabled - soft penalty replaces it
const V3_MAX_LEV_5 = parseInt(process.env.V3_MAX_LEV_5 || '99');  // disabled
...
const IC_W_LEVERAGE = parseFloat(process.env.IC_W_LEVERAGE || '-0.075');
const IC_SCALE = parseFloat(process.env.IC_SCALE || '40');
...
// ===== Stacks-V4 selector (Atlas + bring-back floor + IC composite score) =====
...
function selectStacksICPortfolio(input: AtlasInput): Lineup[] {
  ...
  // === V3: precompute proj_min and n_lev_5 per candidate ===
  const candProjMin = new Float32Array(P);
  const candNLev5 = new Int8Array(P);
  for (let c = 0; c < P; c++) {
    const lu = candidates[c];
    let pmin = Infinity, nlev = 0;
    for (const p of lu.players) {
      const proj = p.projection || 0;
      if (proj < pmin) pmin = proj;
      if ((p.ownership || 0) < 5) nlev++;
    }
    candProjMin[c] = pmin;
    candNLev5[c] = nlev;
  }

  // === Greedy with bring-back floor + V3 filters ===
  const selected: Lineup[] = [];
  const selectedSet = new Set<number>();
  const exposureCount = new Map<string, number>();
  const teamStackCount = new Map<string, number>();
  ...
  for (let step = 0; step < N; step++) {
    ...
    for (let gi = 0; gi < greedyPool.length; gi++) {
      ...
      // V4: soft penalty on proj_min and n_lev_5 (added to score below)
      const lu = candidates[c];
      let okExp = true;
      for (const p of lu.players) {
        const isAnchor = anchorIds.has(p.id);
      ...
      for (const p of lu.players) if (anchorIds.has(p.id)) nAnchors++;
      // V4 soft penalties
      const projMinPenalty = candProjMin[c] < 4.0 ? -V4_PROJ_MIN_PENALTY * (4.0 - candProjMin[c]) : 0;
      const nLevPenalty = candNLev5[c] > 2 ? -V4_NLEV_PENALTY * (candNLev5[c] - 2) : 0;
      const score = candEV[c] - ATLAS_W_MULTI_BLEND * chalkRank[c] + STACKS_ANCHOR_BOOST * nAnchors + projMinPenalty + nLevPenalty;
      if (score > bestScore) { bestScore = score; bestIdx = c; }
    }
    if (bestIdx === -1) {
      // Retry without V3 hard filters (fallback when no candidates remain)
      for (let gi = 0; gi < greedyPool.length; gi++) {
        const c = greedyPool[gi];
        if (selectedSet.has(c)) continue;
        const lu = candidates[c];
        ...
        let nAnchors = 0;
        for (const p of lu.players) if (anchorIds.has(p.id)) nAnchors++;
        const score = candEV[c] - ATLAS_W_MULTI_BLEND * chalkRank[c] + STACKS_ANCHOR_BOOST * nAnchors;
        if (score > bestScore) { bestScore = score; bestIdx = c; }
      }
      if (bestIdx === -1) break;
    }
...
async function main() {
  console.log('================================================================');
  console.log('Atlas vs Stacks-BB vs Stacks-V4 — 29-slate LOO three-way backtest');
  console.log('================================================================\n');
  ...
  console.log('Metric                       Atlas         Stacks-BB         Stacks-V4');
  console.log('---------------------------------------------------------------------------');
  ...
  console.log('Metric                       pro tgt    Atlas              Stacks-BB         Stacks-V4');
  for (const k of UNIVERSAL_METRICS) {
  ...
  console.log('================================================================\n');
  for (const [label, a, l] of [['Atlas', aAgg, aLoo] as const, ['Stacks-BB', vAgg, vLoo] as const, ['Stacks-V4', iAgg, iLoo] as const]) {
