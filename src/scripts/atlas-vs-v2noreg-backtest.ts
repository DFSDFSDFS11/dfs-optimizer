/**
 * Argus-Atlas vs PortfolioCoverage-v2-no-reg — 29-slate head-to-head.
 *
 * Leakage-free comparison. Atlas pipeline ported from _argus_atlas_preslate.ts:
 *   - T-copula sim (1500 worlds, ν=5)
 *   - Uniform field sample (8000 from candidate pool)
 *   - EV-vs-field per candidate via sorted-field rank → payout
 *   - GBM v3 (size-2 combo prior) chalk score
 *   - Greedy: score = candEV - W_MULTI_BLEND * chalkRank
 *   - Exposure caps (hitter 0.25, pitcher 0.45) + team-stack cap (0.20)
 *
 * v2-no-reg uses shared v1-sim-stats module (3000 worlds) + greedy coverage
 * + V1-EV fallback. No ownership regularizer.
 *
 * Both selectors run with Adj Own override applied (preslate-realistic
 * ownership). chalkAnchorOwn for v2 from pool top-100 (Atlas doesn't use it).
 *
 * Reports gate decisions, LOO sensitivity, Mahalanobis on 18-consensus slates.
 */
...
const OUT_JSON = path.join(DIR, 'atlas_vs_v2noreg_results.json');
...
async function main() {
  console.log('================================================================');
  console.log('Atlas vs v2-no-reg — 29-slate head-to-head (leakage-free)');
  console.log('================================================================\n');
  ...
  for (const s of SLATES) {
    ...
    try {
      ...
      const v2 = selectPortfolioCoverageV2Portfolio(
        ...
        { fallbackToV1: true },                          // no chalkAnchorOwn = reg off
      );
      ...
      rows.push({
        ...
        v2nr:  { size: v2.selected.length, roi: v2Tourney.roi, mahal: v2Mahal, top1: v2Tourney.top1, top01: v2Tourney.top01, cost: v2Tourney.cost, payout: v2Tourney.payout, metrics: v2Metrics },
      });
      ...
      console.log(
        ...
        `atlas: ROI=${atlasTourney.roi.toFixed(0).padStart(5)}% mahal=${atlasMahal?.toFixed(2) || ' n/a'} top1=${atlasTourney.top1} | ` +
        `v2nr: ROI=${v2Tourney.roi.toFixed(0).padStart(5)}% mahal=${v2Mahal?.toFixed(2) || ' n/a'} top1=${v2Tourney.top1} | ${ts.toFixed(1)}s`,
      );
    } catch (e: any) {
  ...
  const aAgg = agg('atlas'), vAgg = agg('v2nr');
  const aLoo = loo('atlas'), vLoo = loo('v2nr');

  console.log('Metric                       Atlas              v2-no-reg');
  console.log('---------------------------------------------------------------');
  console.log(`Slates                       ${rows.length}                  ${rows.length}`);
  console.log(`Total cost                   $${aAgg.cost.toLocaleString().padStart(8)}          $${vAgg.cost.toLocaleString().padStart(8)}`);
  console.log(`Total payout                 $${aAgg.pay.toFixed(0).padStart(8)}          $${vAgg.pay.toFixed(0).padStart(8)}`);
  console.log(`Full-sample ROI              ${aAgg.roi.toFixed(2).padStart(8)}%         ${vAgg.roi.toFixed(2).padStart(8)}%`);
  console.log(`Profitable                   ${aAgg.prof}/${rows.length}                ${vAgg.prof}/${rows.length}`);
  console.log(`Top-1% hits                  ${aAgg.top1.toString().padStart(8)}          ${vAgg.top1.toString().padStart(8)}`);
  console.log(`Top-0.1% hits                ${aAgg.top01.toString().padStart(8)}          ${vAgg.top01.toString().padStart(8)}`);
  console.log(`LOO mean ROI                 ${aLoo.mean.toFixed(2).padStart(8)}%         ${vLoo.mean.toFixed(2).padStart(8)}%`);
  console.log(`LOO std                      ${aLoo.std.toFixed(2).padStart(8)}%         ${vLoo.std.toFixed(2).padStart(8)}%`);
  console.log(`LOO worst drop               ${aLoo.worst.toFixed(2).padStart(8)}%         ${vLoo.worst.toFixed(2).padStart(8)}%`);
  console.log(`LOO worst slate              ${aLoo.worstSlate.padEnd(14)}     ${vLoo.worstSlate}`);
  console.log(`Mean Mahalanobis             ${aAgg.meanD.toFixed(3).padStart(8)}          ${vAgg.meanD.toFixed(3).padStart(8)}      (target <1.5)`);
  console.log(`Slates d<1.5                 ${aAgg.dLt15}/${aAgg.distsN}               ${vAgg.dLt15}/${vAgg.distsN}`);
  console.log(`Slates d<2.0                 ${aAgg.dLt20}/${aAgg.distsN}              ${vAgg.dLt20}/${vAgg.distsN}`);

  console.log('\n--- 5-principle structural fidelity ---');
  console.log('Metric                       pro target     Atlas               v2-no-reg');
  for (const k of UNIVERSAL_METRICS) {
    ...
    console.log(`  ${k.padEnd(28)} ${tgt.toString().padStart(8)}  | ${aM.toFixed(3).padStart(8)} (CV ${aCV.toFixed(3)})  | ${vM.toFixed(3).padStart(8)} (CV ${vCV.toFixed(3)})`);
  }

  fs.writeFileSync(OUT_JSON, JSON.stringify({ runDate: new Date().toISOString(), nSlates: rows.length, rows, atlas: { agg: aAgg, loo: aLoo }, v2nr: { agg: vAgg, loo: vLoo } }, null, 2));
  ...
  console.log('================================================================\n');
  for (const [label, a, l] of [['Atlas', aAgg, aLoo] as const, ['v2-no-reg', vAgg, vLoo] as const]) {
