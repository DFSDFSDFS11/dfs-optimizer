/**
 * Argus-v5 LOO validation: GBM-predicted field freqs replace pool-count.
 *
 * For each held-out slate i:
 *   1. Train GBM (depth=4, 150 trees, lr=0.05) on the other 15 slates.
 *   2. For every combo in slate i's candidate pool, compute predicted freq
 *      using GBM (features: log_pool, log_own, sameTeam, log_proj, log_sal).
 *   3. Run Argus selection with GBM predictions as the field-freq input to
 *      the median-rescaled multi-combo penalty.
 *   4. Score Mahalanobis to pro consensus + real DK GPP payout ROI.
 *
 * Compare to Argus-v4 (pool-count) on the same held-out slates.
 *
 * Output: per-slate + aggregate, plus models written to argus_v5_loo/.
 */
...
const GBM = { N_TREES: 150, MAX_DEPTH: 4, LEARNING_RATE: 0.05, MIN_LEAF: 200, N_BINS: 32, L2_LAMBDA: 1.0 };
...
interface ComboTrainRecord { predFreq: number; ownProduct: number; actualFreq: number; sameTeam: boolean; projSum: number; salSum: number; }
interface SlateData {
  slate: string;
  poolSize: number;
  fieldSize: number;
...
function featurizeRaw(predFreq: number, ownProduct: number, sameTeam: boolean, projSum: number, salSum: number): number[] {
  return [
...
function selectArgus(slate: SlateData, fieldFreqs: FieldFreqs): ScoredLineup[] {
...
function buildFieldFreqsGBM(slate: SlateData, gbmModels: Record<number, GBMModel>): FieldFreqs {
  ...
  function predictForCombo(size: number, key: string): number {
    const ids = key.split('|');
    let ownProd = 1, projSum = 0, salSum = 0; const teamCounts = new Map<string, number>();
    for (const id of ids) {
...
interface SlateResult {
  ...
  v4_pay: number; v5_pay: number;
  v4_roi: number; v5_roi: number;
  v4_t1: number; v5_t1: number;
  v4_mahal: number | null; v5_mahal: number | null;
  v4_meanOwn: number; v5_meanOwn: number;
  v4_meanProj: number; v5_meanProj: number;
}

async function main() {
  console.log('=== Argus-v5 (GBM field model) LOO validation, 16 dev slates ===\n');
  ...
  for (let i = 0; i < slates.length; i++) {
    ...
    // Train GBM per size on the 15 training slates.
    ...
    // Build field freqs both ways and run Argus selection.
    const ffPool = buildFieldFreqsPool(heldOut);
    const ffGBM = buildFieldFreqsGBM(heldOut, gbmModels);

    const v4Sel = selectArgus(heldOut, ffPool);
    const v5Sel = selectArgus(heldOut, ffGBM);

    // Compute Mahalanobis + payout.
    ...
    const v5M = computeUniversal(v5Sel.map(s => s.lu.players), slateStats);
    const v4Mahal = computeMahalanobis(heldOut, v4M, slateStats);
    const v5Mahal = computeMahalanobis(heldOut, v5M, slateStats);
    const v4Pay = scorePayout(v4Sel.map(s => s.lu), heldOut);
    const v5Pay = scorePayout(v5Sel.map(s => s.lu), heldOut);

    const v4MeanOwn = mean(v4Sel.map(s => s.lu.ownership));
    const v5MeanOwn = mean(v5Sel.map(s => s.lu.ownership));
    const v4MeanProj = mean(v4Sel.map(s => s.lu.projection));
    const v5MeanProj = mean(v5Sel.map(s => s.lu.projection));

    results.push({
      slate: heldOut.slate, F: heldOut.fieldSize, P: heldOut.poolSize,
      v4_pay: v4Pay.totalPayout, v5_pay: v5Pay.totalPayout,
      v4_roi: (v4Pay.totalPayout / cost - 1) * 100, v5_roi: (v5Pay.totalPayout / cost - 1) * 100,
      v4_t1: v4Pay.t1, v5_t1: v5Pay.t1,
      v4_mahal: v4Mahal, v5_mahal: v5Mahal,
      v4_meanOwn: v4MeanOwn, v5_meanOwn: v5MeanOwn,
      v4_meanProj: v4MeanProj, v5_meanProj: v5MeanProj,
    });
    console.log(`${((Date.now() - t0) / 1000).toFixed(0)}s | v4 pay=$${v4Pay.totalPayout.toFixed(0).padStart(5)} mahal=${v4Mahal?.toFixed(2).padStart(5) || ' n/a'} | v5 pay=$${v5Pay.totalPayout.toFixed(0).padStart(5)} mahal=${v5Mahal?.toFixed(2).padStart(5) || ' n/a'}`);
  }

  // Aggregate.
  const cost = N * FEE * results.length;
  const v4Pay = results.reduce((s, r) => s + r.v4_pay, 0);
  const v5Pay = results.reduce((s, r) => s + r.v5_pay, 0);
  const v4Mahal = results.map(r => r.v4_mahal).filter((x): x is number => x !== null);
  const v5Mahal = results.map(r => r.v5_mahal).filter((x): x is number => x !== null);
  const v4ROI = (v4Pay / cost - 1) * 100;
  const v5ROI = (v5Pay / cost - 1) * 100;

  console.log('\n================================================================');
  console.log('AGGREGATE — Argus-v4 (pool) vs Argus-v5 (GBM field model)');
  console.log('================================================================');
  console.log(`Slates: ${results.length}, total cost: $${cost}`);
  console.log(``);
  console.log(`           |   v4 (pool)   |   v5 (GBM)`);
  console.log(`-----------+---------------+---------------`);
  console.log(`Total pay  |  $${v4Pay.toFixed(0).padStart(8)}    |  $${v5Pay.toFixed(0).padStart(8)}`);
  console.log(`Aggregate ROI | ${v4ROI.toFixed(2).padStart(7)}%    | ${v5ROI.toFixed(2).padStart(7)}%`);
  console.log(`Profitable | ${results.filter(r => r.v4_pay > N * FEE).length}/${results.length}        | ${results.filter(r => r.v5_pay > N * FEE).length}/${results.length}`);
  console.log(`Total t1   | ${results.reduce((s, r) => s + r.v4_t1, 0).toString().padStart(4)}        | ${results.reduce((s, r) => s + r.v5_t1, 0).toString().padStart(4)}`);
  console.log(`Mahal mean | ${mean(v4Mahal).toFixed(3).padStart(8)}     | ${mean(v5Mahal).toFixed(3).padStart(8)}`);
  console.log(`Mahal med  | ${median(v4Mahal).toFixed(3).padStart(8)}     | ${median(v5Mahal).toFixed(3).padStart(8)}`);
  console.log(`d<1.5 gate | ${v4Mahal.filter(d => d < 1.5).length}/${v4Mahal.length}        | ${v5Mahal.filter(d => d < 1.5).length}/${v5Mahal.length}`);
  console.log(`d<2.0      | ${v4Mahal.filter(d => d < 2.0).length}/${v4Mahal.length}        | ${v5Mahal.filter(d => d < 2.0).length}/${v5Mahal.length}`);
  console.log(`Mean own%  | ${mean(results.map(r => r.v4_meanOwn)).toFixed(2).padStart(7)}     | ${mean(results.map(r => r.v5_meanOwn)).toFixed(2).padStart(7)}`);
  console.log(`Mean proj  | ${mean(results.map(r => r.v4_meanProj)).toFixed(2).padStart(7)}     | ${mean(results.map(r => r.v5_meanProj)).toFixed(2).padStart(7)}`);

  console.log('\n--- Per-slate ---');
  console.log('slate          | v4 ROI%  v5 ROI%  Δ%      | v4 mahal  v5 mahal  Δ');
  for (const r of results) {
    const dRoi = r.v5_roi - r.v4_roi;
    const dMahal = (r.v5_mahal !== null && r.v4_mahal !== null) ? r.v5_mahal - r.v4_mahal : null;
    console.log(
      `${r.slate.padEnd(14)} | ${r.v4_roi.toFixed(1).padStart(7)}% ${r.v5_roi.toFixed(1).padStart(7)}% ${(dRoi >= 0 ? '+' : '') + dRoi.toFixed(1) + '%'} | ` +
      `${r.v4_mahal?.toFixed(2).padStart(7) || '   n/a'} ${r.v5_mahal?.toFixed(2).padStart(7) || '   n/a'} ${dMahal !== null ? (dMahal >= 0 ? '+' : '') + dMahal.toFixed(2) : 'n/a'}`
    );
  }

  // Save CSV.
  if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });
  const csvLines = ['slate,F,P,v4_pay,v5_pay,v4_roi,v5_roi,v4_t1,v5_t1,v4_mahal,v5_mahal,v4_meanOwn,v5_meanOwn'];
  for (const r of results) {
    csvLines.push([r.slate, r.F, r.P, r.v4_pay.toFixed(2), r.v5_pay.toFixed(2),
      r.v4_roi.toFixed(2), r.v5_roi.toFixed(2),
      r.v4_t1, r.v5_t1, r.v4_mahal ?? '', r.v5_mahal ?? '',
      r.v4_meanOwn.toFixed(2), r.v5_meanOwn.toFixed(2)].join(','));
  }
  fs.writeFileSync(path.join(OUT_DIR, 'argus_v5_loo.csv'), csvLines.join('\n'));
  console.log(`\nSaved per-slate results to ${path.join(OUT_DIR, 'argus_v5_loo.csv')}`);
}
