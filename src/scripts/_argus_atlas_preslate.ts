...
const TAG = process.env.ARGUS_OUTPUT_TAG || 'atlas';
...
async function main() {
  console.log('================================================================');
  console.log('ARGUS-ATLAS pre-slate (t-copula + uniform-field + GBM combo prior)');
  console.log('================================================================');
  ...
  const outDk = path.join(DIR, `theory_dfs_argus_atlas_preslate_${selected.length}${tagSuffix}.csv`);
  const outDetail = path.join(DIR, `theory_dfs_argus_atlas_preslate_${selected.length}${tagSuffix}_detailed.csv`);
