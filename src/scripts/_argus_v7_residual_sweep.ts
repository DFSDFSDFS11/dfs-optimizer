...
function selectArgus(slate: SlateData, fieldFreqs: FieldFreqs, wMulti: number = ARGUS.W_MULTI): ScoredLineup[] {
...
const W_MULTI_SWEEP = [0.20, 0.30, 0.40, 0.60];
...
interface SlateResult {
  ...
  sweep: Record<number, { sel: ScoredLineup[]; mahal: number | null; pay: number; meanOwn: number; meanProj: number; t1: number; }>;
  optInfo: OptInfo;
}
...
async function main() {
  ...
  console.log(`W_MULTI sweep: [${W_MULTI_SWEEP.join(', ')}]\n`);
  ...
  for (let i = 0; i < slates.length; i++) {
    ...
    // v7 sweep over W_MULTI values.
    const sweep: Record<number, { sel: ScoredLineup[]; mahal: number | null; pay: number; meanOwn: number; meanProj: number; t1: number; }> = {};
    for (const w of W_MULTI_SWEEP) {
      const sel = selectArgus(heldOut, ffGBMv7, w);
      ...
      sweep[w] = {
        sel, mahal, pay: pay.totalPayout, t1: pay.t1,
    ...
    let line = `${((Date.now() - t0) / 1000).toFixed(0)}s | v4 ROI=${((v4Pay.totalPayout / cost - 1) * 100).toFixed(0).padStart(5)}% mahal=${v4Mahal?.toFixed(2).padStart(5) || ' n/a'}`;
    for (const w of W_MULTI_SWEEP) {
      line += ` | W${w.toFixed(2)} ROI=${((sweep[w].pay / cost - 1) * 100).toFixed(0).padStart(5)}% mahal=${sweep[w].mahal?.toFixed(2).padStart(5) || ' n/a'}`;
    }
    console.log(line);
  }
  ...
  console.log('\n================================================================');
  console.log('AGGREGATE — Argus-v4 (pool, W=0.20) vs Argus-v7 (GBM-residual sweep)');
  console.log('================================================================');
  console.log(`Slates: ${results.length}, total cost: $${cost}\n`);

  // Header
  let hdr = 'metric         |  v4 pool';
  for (const w of W_MULTI_SWEEP) hdr += `  | v7 W=${w.toFixed(2)}`;
  console.log(hdr);
  console.log('-'.repeat(hdr.length));

  function row(label: string, valFn: (r: SlateResult, kind: string) => number, fmt: (x: number) => string): string {
    let s = label.padEnd(15) + '|';
    s += '  ' + fmt(valFn(results[0], 'AGG_v4'));  // dummy; we'll override below
    return s;
  }

  // Total pay
  let line = 'Total pay      |  $' + v4Pay.toFixed(0).padStart(8);
  for (const w of W_MULTI_SWEEP) {
    const pay = results.reduce((s, r) => s + (r.sweep[w]?.pay || 0), 0);
    line += `  | $${pay.toFixed(0).padStart(8)}`;
  }
  console.log(line);

  // Aggregate ROI
  line = 'ROI %          |  ' + v4ROI.toFixed(2).padStart(8) + '%';
  for (const w of W_MULTI_SWEEP) {
    const pay = results.reduce((s, r) => s + (r.sweep[w]?.pay || 0), 0);
    const roi = (pay / cost - 1) * 100;
    line += `  | ${roi.toFixed(2).padStart(8)}%`;
  }
  console.log(line);

  // Profitable
  line = 'Profitable     |  ' + results.filter(r => r.v4_pay > N * FEE).length + '/' + results.length;
  for (const w of W_MULTI_SWEEP) {
    const p = results.filter(r => (r.sweep[w]?.pay || 0) > N * FEE).length;
    line += `        | ${p}/${results.length}`;
  }
  console.log(line);

  // Total t1
  line = 'Total t1       |  ' + results.reduce((s, r) => s + r.v4_t1, 0).toString().padStart(7);
  for (const w of W_MULTI_SWEEP) {
    const t = results.reduce((s, r) => s + (r.sweep[w]?.t1 || 0), 0);
    line += `  | ${t.toString().padStart(8)}`;
  }
  console.log(line);

  // Mahal med
  line = 'Mahal med      |  ' + median(v4Mahal).toFixed(3).padStart(8);
  for (const w of W_MULTI_SWEEP) {
    const m = results.map(r => r.sweep[w]?.mahal).filter((x): x is number => x !== null && x !== undefined);
    line += `  | ${median(m).toFixed(3).padStart(8)}`;
  }
  console.log(line);

  // d<1.5 gate
  line = 'd<1.5 gate     |  ' + v4Mahal.filter(d => d < 1.5).length + '/' + v4Mahal.length;
  for (const w of W_MULTI_SWEEP) {
    const m = results.map(r => r.sweep[w]?.mahal).filter((x): x is number => x !== null && x !== undefined);
    line += `        | ${m.filter(d => d < 1.5).length}/${m.length}`;
  }
  console.log(line);

  // Mean own%
  line = 'Mean own%      |  ' + mean(results.map(r => r.v4_meanOwn)).toFixed(2).padStart(8);
  for (const w of W_MULTI_SWEEP) {
    const v = mean(results.map(r => r.sweep[w]?.meanOwn || 0));
    line += `  | ${v.toFixed(2).padStart(8)}`;
  }
  console.log(line);

  // Mean proj
  line = 'Mean proj      |  ' + mean(results.map(r => r.v4_meanProj)).toFixed(2).padStart(8);
  for (const w of W_MULTI_SWEEP) {
    const v = mean(results.map(r => r.sweep[w]?.meanProj || 0));
    line += `  | ${v.toFixed(2).padStart(8)}`;
  }
  console.log(line);

  // Slate-level optimal vs ours.
  console.log('\n--- Per-slate: actual top-1% lineup proj/own vs Argus portfolios ---');
  console.log('slate          | F      | top-1% proj  top-1% own | v4 proj  v4 own | v7 W20 proj W20 own | v7 W40 proj W40 own | v7 W60 proj W60 own');
  for (const r of results) {
    const o = r.optInfo;
    const w20 = r.sweep[0.20]; const w40 = r.sweep[0.40]; const w60 = r.sweep[0.60];
    console.log(
      `${r.slate.padEnd(14)} | ${r.F.toString().padStart(6)} | ${o.topActualProj.toFixed(1).padStart(11)}  ${o.topActualOwn.toFixed(1).padStart(8)}% | ` +
      `${r.v4_meanProj.toFixed(1).padStart(7)}  ${r.v4_meanOwn.toFixed(1).padStart(5)}% | ` +
      `${w20.meanProj.toFixed(1).padStart(10)} ${w20.meanOwn.toFixed(1).padStart(6)}% | ` +
      `${w40.meanProj.toFixed(1).padStart(10)} ${w40.meanOwn.toFixed(1).padStart(6)}% | ` +
      `${w60.meanProj.toFixed(1).padStart(10)} ${w60.meanOwn.toFixed(1).padStart(6)}%`
    );
  }

  // Per-slate ROI.
  console.log('\n--- Per-slate ROI ---');
  let header = 'slate          | v4 pool   ';
  for (const w of W_MULTI_SWEEP) header += `| v7 W=${w.toFixed(2)} `;
  console.log(header);
  for (const r of results) {
    let l = `${r.slate.padEnd(14)} | ${r.v4_roi.toFixed(1).padStart(7)}%   `;
    for (const w of W_MULTI_SWEEP) {
      const roi = ((r.sweep[w].pay / (N * FEE)) - 1) * 100;
      l += `| ${roi.toFixed(1).padStart(7)}%  `;
    }
    console.log(l);
  }
  ...
  for (const w of W_MULTI_SWEEP) {
    headers.push(`v7_W${w}_pay`, `v7_W${w}_roi`, `v7_W${w}_t1`, `v7_W${w}_mahal`, `v7_W${w}_meanOwn`, `v7_W${w}_meanProj`);
  }
  const csvLines = [headers.join(',')];
  for (const r of results) {
    const cols = [r.slate, r.F, r.P, r.optInfo.topActualProj.toFixed(2), r.optInfo.topActualOwn.toFixed(2),
      r.v4_pay.toFixed(2), r.v4_roi.toFixed(2), r.v4_mahal ?? '', r.v4_meanOwn.toFixed(2), r.v4_meanProj.toFixed(2)];
    for (const w of W_MULTI_SWEEP) {
      const sw = r.sweep[w];
      cols.push(sw.pay.toFixed(2), ((sw.pay / (N * FEE) - 1) * 100).toFixed(2), String(sw.t1),
        sw.mahal ?? '', sw.meanOwn.toFixed(2), sw.meanProj.toFixed(2));
    }
    csvLines.push(cols.join(','));
  }
  fs.writeFileSync(path.join(OUT_DIR, 'argus_v7_residual_sweep.csv'), csvLines.join('\n'));
  console.log(`\nSaved per-slate results to ${path.join(OUT_DIR, 'argus_v7_residual_sweep.csv')}`);
}
