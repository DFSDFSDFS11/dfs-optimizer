/**
 * Argus-Atlas vs Argus-Stacks — 29-slate head-to-head LOO backtest.
 *
 * Atlas (baseline) — flat caps + GBM chalk penalty.
 * Stacks (challenger) — Atlas + anchor caps + bring-back floor + leverage-stack floor.
 *
 * Per Theory of DFS Ch.8: optimize at LINEUP level (stack shape, bring-back,
 * leverage team-stacks), not at per-player exposure level.
 */
...
const STACKS_LEVERAGE_BOOST = parseFloat(process.env.STACKS_LEVERAGE_BOOST || '0');  // 0 = bring-back-only mode
const STACKS_LEVERAGE_TOP_PCT = 0.40;
const STACKS_LEVERAGE_FLOOR = parseFloat(process.env.STACKS_LEVERAGE_FLOOR || '0');  // 0 = no leverage floor
...
async function main() {
  console.log('================================================================');
  console.log('Atlas vs Stacks — 29-slate LOO backtest (lineup-level shaping)');
  console.log('================================================================\n');
  ...
  for (const s of SLATES) {
    ...
    try {
      ...
      console.log(
        ...
        `stacks: ROI=${stacksTourney.roi.toFixed(0).padStart(5)}% mahal=${stacksMahal?.toFixed(2) || ' n/a'} top1=${stacksTourney.top1} | ${ts.toFixed(1)}s`,
      );
    } catch (e: any) {
  ...
  console.log('Metric                       Atlas              Stacks');
  console.log('---------------------------------------------------------------');
  ...
  console.log('Metric                       pro target     Atlas               Stacks');
  for (const k of UNIVERSAL_METRICS) {
  ...
  console.log('================================================================\n');
  for (const [label, a, l] of [['Atlas', aAgg, aLoo] as const, ['Stacks', vAgg, vLoo] as const]) {
