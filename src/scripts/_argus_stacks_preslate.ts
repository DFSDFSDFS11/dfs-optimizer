/**
 * Argus-Stacks pre-slate: Atlas + LINEUP-LEVEL portfolio shaping.
 *
 * Same architecture as Atlas (t-copula, EV-vs-uniform-field, GBM v3 combo prior)
 * PLUS portfolio-level constraints derived from live audit of pro 150-lineup portfolios:
 *
 *   1. Anchor caps (from Argus-Anchor): top-N hitters/pitchers by projection get cap raised
 *   2. BRING-BACK FLOOR: portfolio must contain >= BRINGBACK_TARGET fraction of lineups
 *      with at least 1 hitter from the primary stack team's opponent (Ch.8 game-stack).
 *
 * Motivation: live-audit (live_audit/FINDINGS.md) showed pros build bring-back lineups
 * in median 17% of their 150-lineup portfolios (p25=9%, p75=28%) while Atlas builds
 * them in only 7%. This is the biggest LINEUP-LEVEL gap. Per Theory of DFS Ch.8:
 *   "You may choose to have high exposure to a higher projected under-own starting
 *    pitcher and build lineups with him in higher frequency, yet at the same time
 *    build a lineup or several lineups that do not have this starting pitcher and
 *    actually have 5 opposing hitters against him."
 *
 * Env vars:
 ...
 *   ARGUS_OUTPUT_TAG    default 'stacks'
 *   ARGUS_W_MULTI_BLEND default 10
 *   ARGUS_NUM_WORLDS    default 1500
 *   ARGUS_FIELD_SIZE    default 8000
 *   ARGUS_HITTER_CAP    default 0.25      (non-anchor hitters)
 *   ARGUS_PITCHER_CAP   default 0.45      (non-anchor pitchers)
 *   ARGUS_TEAM_CAP      default 0.20
 *   ARGUS_ANCHOR_HIT_CAP default 0.55     (anchor hitters)
 *   ARGUS_ANCHOR_PIT_CAP default 0.70     (anchor pitchers)
 *   ARGUS_N_HIT_ANCHORS  default 3
 *   ARGUS_N_PIT_ANCHORS  default 1
 *   ARGUS_ANCHOR_BOOST   default 5.0      ($ added per anchor in lineup)
 *   ARGUS_BRINGBACK_TARGET default 0.17   (pro median bring-back ≥1 rate)
 *   ARGUS_BRINGBACK_2_TARGET default 0.08 (pro median bring-back ≥2 rate)
 */
...
const POOL_FILES = (process.env.ARGUS_POOL_FILES || 'sspool1pre.csv,sspool2pre.csv,sspool3pre.csv').split(',').map(s => s.trim()).filter(Boolean);
const N = process.env.ARGUS_TARGET_COUNT ? parseInt(process.env.ARGUS_TARGET_COUNT, 10) : 150;
const TAG = process.env.ARGUS_OUTPUT_TAG || 'stacks';
...
const ANCHOR_HIT_CAP = process.env.ARGUS_ANCHOR_HIT_CAP ? parseFloat(process.env.ARGUS_ANCHOR_HIT_CAP) : 0.55;
const ANCHOR_PIT_CAP = process.env.ARGUS_ANCHOR_PIT_CAP ? parseFloat(process.env.ARGUS_ANCHOR_PIT_CAP) : 0.70;
const N_HIT_ANCHORS = process.env.ARGUS_N_HIT_ANCHORS ? parseInt(process.env.ARGUS_N_HIT_ANCHORS, 10) : 3;
const N_PIT_ANCHORS = process.env.ARGUS_N_PIT_ANCHORS ? parseInt(process.env.ARGUS_N_PIT_ANCHORS, 10) : 1;
const ANCHOR_BOOST = process.env.ARGUS_ANCHOR_BOOST ? parseFloat(process.env.ARGUS_ANCHOR_BOOST) : 5.0;
// BB25 validated as peak via 29-slate LOO + BB sweep (2026-05-17 methodology overnight run):
//   BB17 +61.12% | BB20 +68.79% | BB25 +71.07% (PEAK) | BB30 +62.59% | BB35 +15.96% (cliff)
// BB25 = 29/29 LOO-robust, +10.95pp over Atlas.
const BRINGBACK_TARGET = process.env.ARGUS_BRINGBACK_TARGET ? parseFloat(process.env.ARGUS_BRINGBACK_TARGET) : 0.25;
const BRINGBACK_2_TARGET = process.env.ARGUS_BRINGBACK_2_TARGET ? parseFloat(process.env.ARGUS_BRINGBACK_2_TARGET) : 0.12;
const BRINGBACK_3_TARGET = process.env.ARGUS_BRINGBACK_3_TARGET ? parseFloat(process.env.ARGUS_BRINGBACK_3_TARGET) : 0.05;
// Leverage-stack tilt: OFF by default. 29-slate backtest showed leverage tilt over-corrected (Mahal 2.07 → 2.97, ROI 54.75 → 51.02).
// Bring-back floor alone is the winning intervention (ROI 54.75 → 61.12, Mahal 2.07 → 1.95).
const LEVERAGE_STACK_BOOST = process.env.ARGUS_LEVERAGE_STACK_BOOST ? parseFloat(process.env.ARGUS_LEVERAGE_STACK_BOOST) : 0;
const LEVERAGE_STACK_TOP_PCT = process.env.ARGUS_LEVERAGE_STACK_TOP_PCT ? parseFloat(process.env.ARGUS_LEVERAGE_STACK_TOP_PCT) : 0.40;
const LEVERAGE_STACK_FLOOR = process.env.ARGUS_LEVERAGE_STACK_FLOOR ? parseFloat(process.env.ARGUS_LEVERAGE_STACK_FLOOR) : 0;
...
async function main() {
  console.log('================================================================');
  console.log('ARGUS-STACKS pre-slate (Atlas + anchor caps + bring-back floor)');
  console.log('================================================================');
  ...
  console.log(`Anchor caps: hit=${ANCHOR_HIT_CAP} pit=${ANCHOR_PIT_CAP} | top-${N_HIT_ANCHORS} hitters, top-${N_PIT_ANCHORS} pitchers by projection | EV boost $${ANCHOR_BOOST}/anchor`);
  console.log(`Bring-back targets: >=1 in ${(BRINGBACK_TARGET*100).toFixed(0)}%, >=2 in ${(BRINGBACK_2_TARGET*100).toFixed(0)}%, >=3 in ${(BRINGBACK_3_TARGET*100).toFixed(0)}%\n`);
  ...
  console.log(`Merged: ${P} unique lineups (from ${total})\n`);

  // ANCHOR IDENTIFICATION — top-N hitters and top-N pitchers by projection.
  // Restrict to players actually present in the candidate pool (at least one lineup).
  ...
  const anchorHitters = hitterCandidates.slice(0, N_HIT_ANCHORS);
  const anchorPitchers = pitcherCandidates.slice(0, N_PIT_ANCHORS);
  for (const p of anchorHitters) anchorIds.add(p.id);
  for (const p of anchorPitchers) anchorIds.add(p.id);
  console.log(`Anchor hitters (cap ${(ANCHOR_HIT_CAP*100).toFixed(0)}%):`);
  for (const p of anchorHitters) console.log(`  ${p.name.padEnd(26)} ${(p.team||'').padEnd(4)} proj=${(p.projection||0).toFixed(1)} own=${(p.ownership||0).toFixed(1)}% sal=$${p.salary}`);
  console.log(`Anchor pitchers (cap ${(ANCHOR_PIT_CAP*100).toFixed(0)}%):`);
  for (const p of anchorPitchers) console.log(`  ${p.name.padEnd(26)} ${(p.team||'').padEnd(4)} proj=${(p.projection||0).toFixed(1)} own=${(p.ownership||0).toFixed(1)}% sal=$${p.salary}`);
  console.log();

  // Precompute per-candidate: primary stack team, primary team's opponent, bring-back count.
  // bringback_count = number of hitters in lineup from primary stack team's opponent.
  const candBringback = new Int8Array(P);  // 0, 1, 2, 3+ — capped at 4 for int8
  for (let c = 0; c < P; c++) {
    ...
    // Find primary team (max hitters)
    let primaryTeam = '';
    let primaryCount = 0;
    for (const [t, cnt] of teamCount) {
      if (cnt > primaryCount) { primaryCount = cnt; primaryTeam = t; }
    }
    let bb = 0;
    if (primaryTeam && teamOpp.has(primaryTeam)) {
      const opp = teamOpp.get(primaryTeam)!;
      bb = teamCount.get(opp) || 0;
    }
    candBringback[c] = Math.min(bb, 4);
  }
  const totalBB1 = (() => { let n = 0; for (let i = 0; i < P; i++) if (candBringback[i] >= 1) n++; return n; })();
  const totalBB2 = (() => { let n = 0; for (let i = 0; i < P; i++) if (candBringback[i] >= 2) n++; return n; })();
  console.log(`Bring-back availability in pool: ${totalBB1}/${P} (${(totalBB1/P*100).toFixed(1)}%) lineups have BB≥1, ${totalBB2}/${P} (${(totalBB2/P*100).toFixed(1)}%) have BB≥2`);

  // LEVERAGE-STACK identification: per-team top-5 hitter proj_sum / own_sum.
  // Top-quartile teams by (proj/own) ratio are "leverage stack" candidates.
  ...
  const teamLeverageScore = new Map<string, number>();
  for (const [t, r] of teamHitterAgg) {
    r.projs.sort((a, b) => b - a);
    r.owns.sort((a, b) => b - a);
    const top5Proj = r.projs.slice(0, 5).reduce((s, x) => s + x, 0);
    const top5Own = r.owns.slice(0, 5).reduce((s, x) => s + x, 0);
    // Leverage score: high if proj high, own low. Add small constant to avoid div-by-zero.
    const score = top5Proj / Math.max(1, top5Own);
    teamLeverageScore.set(t, score);
  }
  // Top-N teams by leverage score
  const sortedTeams = [...teamLeverageScore.entries()].sort((a, b) => b[1] - a[1]);
  const nLeverageTeams = Math.max(1, Math.ceil(sortedTeams.length * LEVERAGE_STACK_TOP_PCT));
  const leverageTeams = new Set<string>();
  for (let i = 0; i < nLeverageTeams; i++) leverageTeams.add(sortedTeams[i][0]);
  console.log(`Leverage stack teams (top ${nLeverageTeams} of ${sortedTeams.length} by proj/own ratio, bonus $${LEVERAGE_STACK_BOOST} per lineup primary-stacking these):`);
  for (let i = 0; i < Math.min(nLeverageTeams, 8); i++) {
    const [t, score] = sortedTeams[i];
    const agg = teamHitterAgg.get(t)!;
    const p5 = agg.projs.slice(0, 5).reduce((s, x) => s + x, 0);
    const o5 = agg.owns.slice(0, 5).reduce((s, x) => s + x, 0);
    console.log(`  ${t.padEnd(4)} proj=${p5.toFixed(1)} own=${o5.toFixed(1)}% lev=${score.toFixed(2)}`);
  }

  // Per-candidate primary stack team (for leverage scoring)
  ...
  // Greedy selection with bring-back floor enforcement.
  ...
  let bb2Selected = 0;
  let bb3Selected = 0;
  let levSelected = 0;  // count of lineups primary-stacking a leverage team
  for (let step = 0; step < N; step++) {
    const requiredBB1 = Math.ceil(BRINGBACK_TARGET * (step + 1));
    const requiredBB2 = Math.ceil(BRINGBACK_2_TARGET * (step + 1));
    const requiredBB3 = Math.ceil(BRINGBACK_3_TARGET * (step + 1));
    const requiredLev = Math.ceil(LEVERAGE_STACK_FLOOR * (step + 1));
    ...
    for (let gi = 0; gi < greedyPool.length; gi++) {
      ...
      // Floors: BB3 → BB2 → BB1 → leverage. Each restricts candidate pool if behind.
      ...
      for (const p of lu.players) {
        ...
        const cap = isPitcher(p)
          ? (isAnchor ? ANCHOR_PIT_CAP : EXPOSURE_CAP_PITCHER)
          : (isAnchor ? ANCHOR_HIT_CAP : EXPOSURE_CAP_HITTER);
      ...
      const levBonus = leverageTeams.has(candPrimaryTeam[c]) ? LEVERAGE_STACK_BOOST : 0;
      const score = candEV[c] - W_MULTI_BLEND * chalkRank[c] + ANCHOR_BOOST * nAnchors + levBonus;
      if (score > bestScore) { bestScore = score; bestIdx = c; }
    }
    if (bestIdx === -1) {
      // If any floor blocked us, retry without floors.
      if (needBB2 || needBB1 || needLev) {
        for (let gi = 0; gi < greedyPool.length; gi++) {
          const c = greedyPool[gi];
          if (selectedSet.has(c)) continue;
          const lu = candidates[c];
          ...
          let nAnchors = 0;
          for (const p of lu.players) if (anchorIds.has(p.id)) nAnchors++;
          const score = candEV[c] - W_MULTI_BLEND * chalkRank[c] + ANCHOR_BOOST * nAnchors;
          if (score > bestScore) { bestScore = score; bestIdx = c; }
        }
      }
  ...
  console.log(`  Bring-back achieved: BB>=1 ${bb1Selected}/${selected.length} (${(bb1Selected/selected.length*100).toFixed(0)}%), BB>=2 ${bb2Selected}/${selected.length} (${(bb2Selected/selected.length*100).toFixed(0)}%), BB>=3 ${bb3Selected}/${selected.length} (${(bb3Selected/selected.length*100).toFixed(0)}%)`);
  console.log(`  Leverage-stack achieved: ${levSelected}/${selected.length} (${(levSelected/selected.length*100).toFixed(0)}%) lineups primary-stack a leverage team`);
  ...
  console.log('\n================================================================');
  console.log('PORTFOLIO STATS — Argus-Stacks');
  console.log('================================================================');
  ...
  const outDk = path.join(DIR, `theory_dfs_argus_stacks_preslate_${selected.length}${tagSuffix}.csv`);
  const outDetail = path.join(DIR, `theory_dfs_argus_stacks_preslate_${selected.length}${tagSuffix}_detailed.csv`);
  exportForDraftKings(selLineups, config, outDk);
  exportDetailedLineups(selLineups, config, outDetail);
  console.log(`\n================================================================`);
  console.log(`DONE — Argus-Stacks preslate`);
  console.log(`================================================================`);
